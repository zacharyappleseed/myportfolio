        ...
        <>
            <nav className="inline-block lg:mr-24 lg:w-4 fixed left-percentage hidden xl:block">
                ...
                <div className="absolute left-50 transform -translate-x-1/2 space-y-6 mt-36">
                    ...
                    <a href="#" className={`nav-dot selected-circle block w-7 h-7 rounded-full border-4 border-nav ${subCSSHome}`}>
                        <span className="bg-black px-2 py-1 rounded-md ml-10 opacity-0">Home</span>
                    </a>
                    <a href="#arrangments" className={`nav-dot selected-circle block w-7 h-7 rounded-full border-4 border-nav ${subCSSArr}`}>
                        <span className="bg-black px-2 py-1 rounded-md ml-10 opacity-0">Arrangements</span>
                    </a>
                    <a href="#music" className={`nav-dot selected-circle block w-7 h-7 rounded-full border-4 border-nav ${subCSSMusic}`}>
                        <span className="bg-black px-2 py-1 rounded-md ml-10 opacity-0">Music</span>
                    </a>

                    <a href="#contact" className={`nav-dot selected-circle block w-7 h-7 rounded-full border-4 border-nav ${subCSSContact}`}>
